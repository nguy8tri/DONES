...
std::vector<flux> Boundaries(System system, int position) {
    ...
    if(position - system.distro->Width() > 0) {
        ...
        result.push_back({false, position - system.distro->Width(), system.k / system.Dy}); // up
    }
    ...
    if(position + system.distro->Width() >= system.distro->Height() * system.distro->Width()) {
        ...
        result.push_back({false, position + system.distro->Width(), system.k / system.Dy}); // down
    }
    ...
    if(position + system.distro->Width() >= system.distro->Height() * system.distro->Width() || position % system.distro->Width() == 0) {
        result.at(0).fluxMult = result.at(0).fluxMult / 2;
        result.at(1).fluxMult = result.at(1).fluxMult / 2;
    }

    if(position % system.distro->Width() == 0 || (position + 1) % system.distro->Width() == 0) {
